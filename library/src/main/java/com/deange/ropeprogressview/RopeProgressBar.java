...
public class RopeProgressBar extends View {
    ...
    public RopeProgressBar( final Context context,  final AttributeSet attrs, final int defStyleAttr) {
        ...
        mLinesPaint.setStrokeCap(Paint.Cap.ROUND);  //画笔的笔触 方的

        //泡泡
        mBubblePaint.setColor(0xFF5c53e4);// 上方泡泡的颜色
        mBubblePaint.setStyle(Paint.Style.FILL);
        //控制泡泡的四个角是否是圆角
        mBubblePaint.setPathEffect(new CornerPathEffect(dips(2)));

        //泡泡里面的文字
        mTextPaint.setColor(Color.WHITE);//泡泡里面文字的颜色
        mTextPaint.setTextSize(sp(18));//文字的大小
    ...
    protected synchronized void onDraw(final Canvas canvas) {
        ...
        // Draw the bubble text background 画泡泡的背景
        ...
        final float bubbleTop = Math.max(slackHeight, 0);//泡泡的位置
        ...
        final float triangleLeft = clamp(progressEnd - (getTriangleWidth() / 2) - bubbleLeft, 0, getWidth() - getTriangleWidth());
